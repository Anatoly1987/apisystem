...
router.use(function authorization (req,res, next){
    ...
    if(result.data.optionid != MessageCode.user_types.admin){
        result.status='error';
        res.json(result);
        return;
    }
...
router.post("/add_business_apikey", function(req,res){
    var data=req.body;
    BusinessApi_model.findOneAndUpdate({businessid:data.businessid},
    {
        businessid:data.businessid,
        api_sec_key:data.secret_key
    },
    {
        new: true,
    ...
    ,function(err, ApiKey){
        if(err){
            res.json({status:MessageCode.error, error:err, message:'The DB error for updating the business api key'});
            return;
        }
        if(typeof ApiKey.apikeys == 'undefined' || ApiKey.apikeys==null){
            ApiKey.apikeys = [];
        }
        var length = ApiKey.apikeys.length;
        var sitekey = {
            apikey:data.apikey,
        ...
        var apikey_item = null;
        for(var i=0; i<length; i++){
            apikey_item = ApiKey.apikeys[i];
            if(typeof apikey_item.storeid !== 'undefined' && data.storeid == apikey_item.storeid){
                break;
            }   
        }
        if(i == length){
            ...
            apikey_item = ApiKey.apikeys[length];
           // ApiKey.save();
        }

        //add the site...
        length = apikey_item.sites.length;
        for(var i=0; i<length; i++){
            var site = apikey_item.sites[i];
            if(site.sitename == data.sitename){
                apikey_item.sites[i] = sitekey;
                break;
            }
        }
        if(i == length){
            apikey_item.sites.push(sitekey);
        }

        ApiKey.save();

        res.json({
            status:MessageCode.ok , message:'added successfully'
        })
    })
